...
const CartItem = ({ i, onChange, handleDelete, quantityEdit }) => {
    ...
    return (
        <tr key={i.id}>
            ...
            <td>{convertVND(i?.productViewModel.priceAfterDiscount + i?.priceDesign)}</td>
            {quantityEdit ? <td><InputNumber min={1}
                defaultValue={i?.count}
                onChange={(value) => onChange(value, i?.productViewModel?.id)} /></td> : <td>{i?.count}</td>}
            <td>{convertVND((i?.productViewModel.priceAfterDiscount + i?.priceDesign) * i.count)}</td>
            <td>
